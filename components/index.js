...
let properties = JSON.parse(JSON.stringify(BOARD_DATA.filter(s => s.type === 'property' || s.type === 'transport' || s.type === 'utility')));
let opportunityCards = [];
let communityFundCards = [];
let lastDiceRoll = 0;
let isClearingDebt = false;
let debtState = null;
let currentTradePartnerId = null;
let onCardModalOk = null;
let currentAiTradeOffer = null;
let recentlyProposedTrades = new Set();


// Auction State
let isAuctionActive = false;
let auctionProperty = null;
let auctionCurrentBid = 0;
let auctionHighestBidder = null;
...
const tradeReviewModal = document.getElementById('trade-review-modal');
...
const tradeReviewOkBtn = document.getElementById('trade-review-ok-btn');
const casinoBetInput = document.getElementById('casino-bet-amount');
const placeCasinoBetBtn = document.getElementById('place-casino-bet-btn');
const casinoResultDiv = document.getElementById('casino-result');
const closeCasinoModalBtn = document.getElementById('close-casino-modal-btn');
const casinoGameControls = document.getElementById('casino-game-controls');
const reel1 = document.getElementById('reel1');
const reel2 = document.getElementById('reel2');
const reel3 = document.getElementById('reel3');
const reels = [reel1, reel2, reel3];
const aiTradeProposalModal = document.getElementById('ai-trade-proposal-modal');
const acceptAiTradeBtn = document.getElementById('accept-ai-trade-btn');
const rejectAiTradeBtn = document.getElementById('reject-ai-trade-btn');
...
const selectTradePartnerModal = document.getElementById('select-trade-partner-modal');
const tradePartnerList = document.getElementById('trade-partner-list');
const cancelTradePartnerSelectBtn = document.getElementById('cancel-trade-partner-select-btn');
...
function createSpaceElement(spaceData, ...extraClasses) {
...
function renderPropertyImprovements(propertyId) {
...
function updatePlayerTokens() {
    players.forEach(player => {
        ...
        const spaceEl = document.querySelector(`.space[data-id='${player.position}']`);
...
function renderPropertyOwnership() {
    properties.forEach(prop => {
        ...
        if (prop.owner !== undefined) {
            ...
            ownerIndicator.style.backgroundColor = PLAYER_COLORS[prop.owner];
...
// Game Logic
function startGame(numAIPlayers) {
    ...
    players.forEach(player => {
        ...
        `;
        const header = panel.querySelector('.player-name');
...
function handlePayment(player, amount, creditor) {
...
// Property Management
function playerHasMonopoly(player, group) {
...
function isBuildEven(player, propertyToBuildOn) {
...
function isSellEven(player, propertyToSellFrom) {
...
function showManagePropertiesModal() {
    ...
    const debtInfo = document.getElementById('debt-resolution-info');
    const bankruptcyBtn = document.getElementById('declare-bankruptcy-btn');
    const closeModalBtn = document.getElementById('close-manage-modal-btn');
...
function showTradeModal(partnerId) {
    const player = players[currentPlayerIndex];
    ...
    const offerMoneyInput = document.getElementById('offer-money');
    offerMoneyInput.value = '0';
    offerMoneyInput.max = String(player.money);
    const requestMoneyInput = document.getElementById('request-money');
...
function populateTradeList(listId, player) {
...
function handleTradeOffer() {
    ...
    const offerProperties = Array.from(document.querySelectorAll('#offer-properties-list .selected')).map(el => parseInt(el.dataset.propertyId));
    const requestProperties = Array.from(document.querySelectorAll('#request-properties-list .selected')).map(el => parseInt(el.dataset.propertyId));
 ...
 */
function calculateBoardStateScore(propertyIds, money) {
    let score = money;
...
function evaluateTradeOfferByAI(offer) {
...
function showTradeReviewModal(offer, decision) {
    const fromPlayer = players[offer.fromPlayerId];
    const toPlayer = players[offer.toPlayerId];
    
    const formatItems = (money, propIds) => {
        let items = [];
...
async function considerBuildingHouses(player) {
    // Defines the priority of which color group to build on first.
    const MONOPOLY_BUILD_ORDER = [
...
function getTradeOfferKey(offer) {
...
async function considerProposingTrade() {
...
function findBestTradeForAI(aiPlayer, targetPlayer) {
    const potentialMonopolies = new Map();
...
function findPropsToOffer(aiPlayer, requestedProp) {
    const offered = [];
...
function evaluateAIOfferValue(offer) {
...
function showAITradeProposalModal(offer) {
    const fromPlayer = players[offer.fromPlayerId];
