...
type AIPersonality = 'Aggressive' | 'Sarcastic' | 'Wealthy' | 'Desperate' | 'Friendly';
...
const PERSONALITIES: AIPersonality[] = ['Aggressive', 'Sarcastic', 'Wealthy', 'Desperate', 'Friendly'];

// Gemini API Integration
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

async function getAIResponse(prompt: string, model: string = 'gemini-3-flash-preview', systemInstruction?: string) {
    try {
        const response = await ai.models.generateContent({
            model: model,
            contents: prompt,
            config: {
                systemInstruction: systemInstruction || "You are a witty AI player in a city-themed Monopoly game.",
                temperature: 0.8,
            }
        });
...
async function getAISmartTradeDecision(trade: TradeOffer, aiPlayer: Player, fromPlayer: Player, propertiesList: Property[]): Promise<{ accepted: boolean; reasoning: string }> {
    const prompt = `
        Evaluate this trade in "AI City Monopoly". 
        YOU: ${aiPlayer.name} (Personality: ${aiPlayer.personality}). Money: $${aiPlayer.money}. Properties: ${aiPlayer.properties.map(id => propertiesList.find(p => p.id === id)?.name).join(', ')}.
        OFFERER: ${fromPlayer.name}. Money: $${fromPlayer.money}. Properties: ${fromPlayer.properties.map(id => propertiesList.find(p => p.id === id)?.name).join(', ')}.
        THE TRADE: 
        Offerer gives: $${trade.offer.money} and properties [${trade.offer.properties.map(id => propertiesList.find(p => p.id === id)?.name).join(', ')}].
        You give: $${trade.request.money} and properties [${trade.request.properties.map(id => propertiesList.find(p => p.id === id)?.name).join(', ')}].

        Consider monopolies (owning all in a color group), building houses, and cash flow.
        Return JSON format: {"accepted": boolean, "reasoning": "string explanation"}
    `;
    ...
    try {
        ...
        return JSON.parse(response.text || '{"accepted": false, "reasoning": "I have a bad feeling about this."}');
    } catch (e) {
        console.error("Smart Trade Error", e);
        return { accepted: evaluateTradeOfferByAI(trade), reasoning: "My logic circuits are slightly buzzed, but here's my standard response." };
    }
}

async function showAICommentary(player: Player, action: string) {
    if (!player.isAI) return;
    
    const panel = document.getElementById(`player-${player.id}-panel`);
    if (!panel) return;

    let bubble = panel.querySelector('.ai-speech-bubble');
    if (!bubble) {
    ...
    const prompt = `Say something short (under 15 words) as a ${player.personality} player who just ${action}. Current money: $${player.money}.`;
    const comment = await getAIResponse(prompt);

    if (comment) {
        bubble.textContent = comment;
        bubble.classList.add('visible');
        logMessage(`${player.name}: "${comment}"`, player.id);
...
function renderBoard() {
    board.innerHTML = '';
    // Re-render center console since board is cleared
    const consoleEl = document.createElement('div');
    consoleEl.className = 'center-console';
    consoleEl.innerHTML = `
    ...
    // (Actual linking happens via DOM search in methods)
}
...
function updatePlayerTokens() {
    players.forEach(player => {
        ...
        token.style.top = `${spaceRect.top - boardRect.top + offset + 5}px`;
        token.style.left = `${spaceRect.left - boardRect.left + offset + 5}px`;
    });
}
...
function logMessage(msg: string, playerId?: number) {
    const msgLog = document.getElementById('message-log')!;
...
function startGame(numAIPlayers: number) {
    ...
    for (let i = 1; i <= numAIPlayers; i++) {
        players.push({ 
            ...
            isAI: true, 
            properties: [], 
            hasVisitedCasino: false, 
            consecutiveDoubles: 0, 
            isBankrupt: false,
            personality: PERSONALITIES[getRandomInt(0, PERSONALITIES.length - 1)]
        });
    }
    ...
    players.forEach(player => {
        ...
        panel.innerHTML = `
            <h2 class="player-name">${player.name}${player.isAI ? ` (${player.personality})` : ''}</h2>
            <p class="player-money">Money: $1500</p>
            <div class="player-properties"></div>
            ${player.isAI ? '<div class="ai-speech-bubble"></div>' : ''}
        `;
...
async function handleAITurn() {
    const player = players[currentPlayerIndex];
    await handleAIPropertyManagement(); 
    ...
    // AI comment for turn start
    showAICommentary(player, "starts their turn");
...
function handlePayment(player: Player, amount: number, creditor: Player | 'bank'): boolean {
    if (player.money >= amount) {
        ...
        if (player.isAI) showAICommentary(player, `paid $${amount} to ${creditor === 'bank' ? 'the bank' : creditor.name}`);
        return true; 
    } else {
...
function goToJail(player: Player) {
    ...
    if (player.isAI) showAICommentary(player, "was sent to jail");
    updatePlayerTokens();
    endTurn();
}
...
function rollDice() {
    ...
    d1El.textContent = String(d1);
    d2El.textContent = String(d2);
...
function buyProperty() {
    ...
    if (player.money >= prop.price) {
        ...
        if (player.isAI) showAICommentary(player, `bought ${prop.name} for $${prop.price}`);
        updateUI();
    }
    hideModals();
    enableEndTurn();
...
async function handleAICasino() {
    ...
    const winChance = 0.4; // AI has lower win chance than humans for balance
    let winnings = 0;
    const chosenSymbols = [...casinoSymbols].sort(() => 0.5 - Math.random());

    if (Math.random() < winChance) { 
        const threeMatchChance = 0.1;
        const winningSymbol = chosenSymbols[0];
        if (Math.random() < threeMatchChance) { 
    ...
    if (winnings > 0) {
        ...
        showAICommentary(player, `won $${winnings} at the casino! Jackpot!`);
        updatePlayerMoney(player, winnings);
    } else {
        logMessage(`${player.name} lost their bet at the casino.`);
        showAICommentary(player, "lost money at the casino slots");
    }
...
function isBuildEven(player: Player, propertyToBuildOn: Property): boolean {
    if (propertyToBuildOn.houses >= 5) return false; 
...
function isSellEven(player: Player, propertyToSellFrom: Property): boolean {
    if (propertyToSellFrom.houses <= 0) return false; 
...
function buildHouse(propertyId: number) {
    ...
    if (player.money >= prop.houseCost) {
        ...
        checkDebtResolved() || (!player.isAI && showManagePropertiesModal()); 
    } else {
...
function showSelectTradePartnerModal() {
    ...
    players.forEach(p => {
        if (p.id !== currentPlayerIndex && !p.isBankrupt) { 
...
async function handleTradeOffer() {
    ...
    if (toPlayer.isAI) {
        logMessage(`AI is evaluating trade proposal...`);
        const decision = await getAISmartTradeDecision(tradeOffer, toPlayer, fromPlayer, properties);
        if (decision.accepted) {
            executeTrade(tradeOffer);
            showTradeReviewModal(tradeOffer, 'accepted', decision.reasoning);
        } else {
            showTradeReviewModal(tradeOffer, 'rejected', decision.reasoning);
        }
    }
}
...
function evaluateTradeOfferByAI(offer: TradeOffer): boolean {
    ...
    const finalAiGain = aiGain + (offer.offer.properties.length - offer.request.properties.length) * 150;
    return finalAiGain > 0 && finalAiGain > opponentGain * 0.5;
}
...
function showTradeReviewModal(offer: TradeOffer, decision: 'accepted' | 'rejected', reasoning?: string) {
    const fromPlayer = players[offer.fromPlayerId];
    const toPlayer = players[offer.toPlayerId];
    ...
    summaryEl.innerHTML = `
        ...
        ${reasoning ? `<div class="ai-reasoning"><strong>Reasoning:</strong> ${reasoning}</div>` : ''}
    `;
...
function startAuction(property: Property) {
    ...
    auctionParticipants = players.map(p => !p.isBankrupt); 
...
function endAuction() {
    ...
    if (auctionHighestBidder !== null) {
        ...
        if (winner.isAI) showAICommentary(winner, `won ${property.name} in an auction`);
        updateUI();
    } else {
...
function checkDebtResolved(): boolean {
    ...
    if (debtor.money >= debtState.amount) {
        ...
        if (!debtor.isAI) showManagePropertiesModal(); 
        return false;
    }
}
...
function declareBankruptcy() {
    ...
    if (debtor.isAI) showAICommentary(debtor, "went bankrupt. Game over for me!");
...
async function considerBuildingHouses(player: Player) {
    const MONOPOLY_BUILD_ORDER: PropertyGroup[] = ['Red', 'Yellow', 'Green', 'DarkBlue', 'Orange', 'Pink', 'LightBlue', 'Brown'];
    ...
    while (true) {
        ...
        if (bestBuildOption) {
            buildHouse(bestBuildOption.property.id);
    ...
    if (housesBuiltThisTurn) {
        showAICommentary(player, "finished developing their city properties");
    }
}
...
function getTradeOfferKey(offer: TradeOffer): string {
    ...
    return `${offer.fromPlayerId}>${offer.toPlayerId}|oP:${offerProps}|oM:${offer.offer.money}|rP:${requestProps}|rM:${offer.request.money}`;
}

async function considerProposingTrade(): Promise<boolean> {
    const aiPlayer = players[currentPlayerIndex];
    if (aiPlayer.money < 500 || players.flatMap(p => p.properties).length < 8) return false;
    ...
    if (bestTrade) {
        ...
        if (recentlyProposedTrades.has(tradeKey)) return false;
        ...
        if (targetPlayer.isAI) {
            const decision = await getAISmartTradeDecision(bestTrade, targetPlayer, aiPlayer, properties);
            if (decision.accepted) {
                executeTrade(bestTrade);
                showTradeReviewModal(bestTrade, 'accepted', decision.reasoning);
...
function findBestTradeForAI(aiPlayer: Player, targetPlayer: Player): TradeOffer | null {
    ...
    aiPlayer.properties.forEach(pId => {
        const prop = getPropertyById(pId)!;
        if (!potentialMonopolies.has(prop.group)) potentialMonopolies.set(prop.group, []);
    ...
    for (const [group, ownedProps] of potentialMonopolies.entries()) {
        ...
        if (ownedProps.length === groupSize) continue;
        if (groupSize >= 3 && ownedProps.length < 2) continue;
        ...
        if (targetHasMissingProps.length > 0) {
            const requestedProp = targetHasMissingProps[0]; 
            const aiOfferedProps = findPropsToOffer(aiPlayer, requestedProp);
            const requestValue = requestedProp.price * 1.2; 
...
function findPropsToOffer(aiPlayer: Player, requestedProp: Property): Property[] {
    ...
    for (const prop of aiProps) {
        ...
        if (totalValue <= requestedProp.price * 2.5) offered.push(prop);
    }
    return offered;
}
...
function evaluateAIOfferValue(offer: TradeOffer): number {
    ...
    offer.request.properties.forEach(pId => {
        ...
        if (aiWouldHaveAll) score += 500 + prop.price;
    });
...
acceptAiTradeBtn.addEventListener('click', async () => {
